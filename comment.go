...
func (c *Comment) Delete() error {
	...
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/talkbox/guestbooks/%s/comments/%s/flags?locale=%s", c.EpisodeID, c.CommentID, c.crunchy.Locale)
...
func (c *Comment) Reply(message string, spoiler bool) (*Comment, error) {
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/talkbox/guestbooks/%s/comments?locale=%s", c.EpisodeID, c.crunchy.Locale)
	var flags []string
	if spoiler {
...
func (c *Comment) Replies(page uint, size uint) ([]*Comment, error) {
	...
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/talkbox/guestbooks/%s/comments/%s/replies?page_size=%d&page=%d&locale=%s", c.EpisodeID, c.CommentID, size, page, c.Locale)
...
func (c *Comment) vote(voteType, readableName string) error {
	...
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/talkbox/guestbooks/%s/comments/%s/votes?locale=%s", c.EpisodeID, c.CommentID, c.crunchy.Locale)
...
func (c *Comment) unVote(voteType, readableName string) error {
	for i, userVote := range c.UserVotes {
		if userVote == voteType {
			endpoint := fmt.Sprintf("https://www.crunchyroll.com/talkbox/guestbooks/%s/comments/%s/votes?vote_type=%s&locale=%s", c.EpisodeID, c.CommentID, voteType, c.crunchy.Locale)
