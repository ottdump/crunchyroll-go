...
func (or *OwnerReview) Edit(title, content string, spoiler bool) error {
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/content-reviews/v2/en-US/user/%s/review/series/%s", or.crunchy.Config.AccountID, or.SeriesID)
...
func (ur *UserReview) rate(positive bool) error {
	...
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/content-reviews/v2/user/%s/rating/review/%s", ur.crunchy.Config.AccountID, ur.ReviewData.ID)
	var body []byte
	if positive {
...
func (ur *UserReview) Report() error {
	...
	endpoint := fmt.Sprintf("https://www.crunchyroll.com/content-reviews/v2/user/%s/report/review/%s", ur.crunchy.Config.AccountID, ur.ReviewData.ID)
